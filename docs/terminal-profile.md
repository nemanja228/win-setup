...
| `profiles/autohotkey/WtTransparent.ahk` | Staged to `%LocalAppData%\win-setup\autohotkey\WtTransparent.ahk`; startup shortcut in `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\WtTransparent.lnk` points at the staged copy | AHK v2 — toggles window transparency. Auto-launches on logon. Re-staged on rerun when the repo file's content changes (SHA256 compare); the running AHK process for this script is stopped first so the destination is unlocked. Decouples the repo from the running process so the repo stays freely deletable/movable. |
...
If you symlinked, no re-run is needed after editing — the link still points at the repo file. Restart the shell (or reload `$PROFILE`) to pick up profile changes; reload Windows Terminal settings live (no restart needed). For `.ahk` edits the symlink option does NOT apply — the AHK file is always staged (copied) to `%LocalAppData%\win-setup\autohotkey\` regardless of `-Symlink`. Re-run `bootstrap.ps1 -Steps profiles` (or `Install-Profiles.ps1 -Only ahk`) after editing; the step stops the running instance and re-stages the file. Relaunch the Startup shortcut to start it again.
