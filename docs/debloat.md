...
4. **`tweaks.reg`** — fills gaps the above tools miss: Copilot/Recall/Click-to-Do policy, advertising ID, lock-screen suggestions, Storage Sense, Edge preload, classic right-click context menu, Developer mode, default terminal = Windows Terminal, etc. Universal anti-crap + dev-standard defaults — anyone forking probably wants it kept intact. Per-value imported by `Import-RegFilePerValue` so partial failures get visible reports.
4a. **`tweaks.personal.reg`** — maintainer's Windows taste, imported by the same step: regional pack (Serbia/CET/en-US/dd.MM.yyyy/24h/Serbian Latin keyboard), dark mode, left-aligned taskbar + seconds in clock, wallpaper slideshow, OneDrive autostart off, precision-touchpad-as-mouse. Forkers should edit or delete this file.
